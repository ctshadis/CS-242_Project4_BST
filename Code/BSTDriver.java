...
public class BSTDriver {
    public static void main (String args[])  {
       ...
       try {
           ...
           Scanner testFile = new Scanner(new FileReader("TestISBN.txt"));
           ...
           while (inFile.hasNextLine())  {
            ...
            while (testFile.hasNextLine()) {
                ...
                Book result = bst.find(element, searchCount);
                if (result != null) {
            ...
            // now let us delete all the elements from the BST
            inFile = new Scanner (new FileInputStream("Books.txt"));
            int deleteCount = 0;
            while (inFile.hasNextLine())  {
			      line = inFile.nextLine();
			      Book aBook = new Book(line.substring(0,10),
			      ...
			      Integer.parseInt(line.substring(95,99)),
			      line.substring(104,line.length()));
			      if (bst.remove(aBook) ) deleteCount++;
		    }
		    out.println();
		    out.println(deleteCount + " elements have been deleted from the BST");
		    out.println("After deleting all the elements the size of the tree is " +
		         bst.size() );
		    out.println("After deleting all the elements the height of the tree is " +
		         bst.height() );
